...
└────────┬────────┘
         ▼
┌─────────────────┐
│ 6. TEST & APPLY │ ← Apply once the shape has settled, not each loop
└─────────────────┘
```
...
**Do not hand-write types to mirror a tracking plan.** TypeScript, Kotlin and Swift
are all generated by `rudder-cli typer generate`; a hand-maintained mirror drifts
from the plan silently, which is the exact failure this phase exists to prevent.
...
The output is a `RudderTyper` class with `track`-prefixed methods and an interface per
event, with your custom types as shared unions:

```typescript
// generated — do not edit
export type CustomTypeTransformationLanguage = 'javascript' | 'python';

export interface TransformationCreated {
  transformationId: string;
  transformationLanguage: CustomTypeTransformationLanguage;
  templateType?: string;
}
```

See the `rudder-typer-workflow` skill for the rest.
