...
|---|---|
| `rudder-core` | The skill teaches a **cross-tool** RudderStack concept (data modeling, tracking plan design, instrumentation strategy, debugging). Content should apply regardless of whether the user drives via CLI or MCP. |
...
- Tool-plugin seed/workflow skills keep the **`-workflow`** suffix (e.g., `rudder-cli-workflow`, `rudder-mcp-workflow`).
...
5. **Link to the source** — if you're documenting a CLI subcommand or MCP tool, link to its implementation or docs.
