...
- **Before pushing marketplace/layout changes, replay cursor.directory's discovery locally:** `node --experimental-transform-types scripts/cursor-preview/preview.mts` (needs Node ≥ 22.6). This runs cursor.directory's **real** parser — vendored verbatim under `scripts/cursor-preview/` (`parse.ts` + `slug.ts`, pinned to an upstream commit in the file header) — against your working tree, by shimming `fetch` to serve local files instead of GitHub. It prints exactly what cursor.directory would find, or the exact error it would throw. cursor.directory only ingests the **default branch**, so this is the only way to validate without merging to `main`. It also runs in pre-push and CI. To refresh the vendored parser, re-pull `parse.ts`/`slug.ts` at a newer commit and re-apply the one-line `@/lib/slug` → `./slug.ts` import rewrite noted in the header.
